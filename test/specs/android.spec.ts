    ...
    it('should run some simple tests against the sample Android app',
        async () => {
            try {
                ...
                var results = await driver.execute('axe:scan', androidAxeSettings);
                writeJsonToFile('Results/result1.json', results);
                ...
                results = await driver.execute('axe:scan', androidAxeSettings);
                writeJsonToFile('Results/result2.json', results);
                ...
                results = await driver.execute('axe:scan', androidAxeSettings);
                writeJsonToFile('Results/result3.json', results);
            } finally {
                await driver.pause(3000);
